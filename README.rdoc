...
[message]  Name of the field in the JSON record that contains the
           ...
           If message is '', it will automatically be set to 'message' or
           'log' if these fields are present in the log data.
           The parameter is only applicable for log streams that contain JSON
           records.
           Default: ''.

[remove_tag_prefix]  The prefix to remove from the input tag when outputting
            a record. A prefix has to be a complete tag part.
            Example: If remove_tag_prefix is set to 'foo', the input tag
            foo.bar.baz is transformed to bar.baz and the input tag
...
[max_lines]  Maximum number of lines in a detected exception stack trace.
             ...
             trace that has been detected so far will be output as a single
...
[stream]  Name of a field in the JSON record that contains the name of a
          ...
          combined.
          This parameter is ignored if stream is ''.
          The parameter is only applicable for log streams that contain JSON
          records.
          Default: ''.
